...
class DetalleDependenciaSecundariaForm(forms.ModelForm):
    class Meta:
        model = DetalleDependenciaSecundaria  # Asocia el formulario con el modelo DetalleServicioEmergencia
        fields = ['dependencia_secundaria', 'numero_movil_secundaria', 'nombre_a_cargo_secundaria']  # Define los campos que serán incluidos en el formulario
        widgets = {
            'dependencia_secundaria': forms.HiddenInput(),  # Campo oculto para el servicio de emergencia
            'numero_movil_secundaria': forms.TextInput(attrs={'class': 'bg-gray-50 dark:bg-gray-800 dark:text-gray-200 border border-gray-300 dark:border-gray-600 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-3'}),  # Campo de texto para el número móvil con estilos
            'nombre_a_cargo_secundaria': forms.TextInput(attrs={'class': 'bg-gray-50 dark:bg-gray-800 dark:text-gray-200 border border-gray-300 dark:border-gray-600 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-3'}),  # Campo de texto para el nombre a cargo con estilos
        }

 #Formulario para DetalleDependenciaSecundaria
class DetalleInstitucionFederalForm(forms.ModelForm):
    class Meta:
        model = DetalleInstitucionFederal  # Asocia el formulario con el modelo DetalleInstitucionFederal
        fields = ['institucion_federal', 'numero_movil_federal', 'nombre_a_cargo_federal']  # Define los campos que serán incluidos en el formulario
        widgets = {
            'institucion_federal': forms.HiddenInput(),  # Campo oculto para el institucion federal
            'numero_movil_federal': forms.TextInput(attrs={'class': 'bg-gray-50 dark:bg-gray-800 dark:text-gray-200 border border-gray-300 dark:border-gray-600 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-3'}),  # Campo de texto para el número móvil con estilos
            'nombre_a_cargo_federal': forms.TextInput(attrs={'class': 'bg-gray-50 dark:bg-gray-800 dark:text-gray-200 border border-gray-300 dark:border-gray-600 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-3'}),  # Campo de texto para el nombre a cargo con estilos
        }
...
class BaseComisariaForm(forms.ModelForm):
    ...
    instituciones_hospitalarias = forms.ModelMultipleChoiceField(
     ...
     # Campo para seleccionar múltiples Instituciones federals
    instituciones_federales = forms.ModelMultipleChoiceField(
        queryset=InstitucionesFederales.objects.all(),  # Define el conjunto de opciones para el campo
        required=False,  # El campo no es obligatorio
        label='Instituciones Federales',  # Etiqueta del campo
        widget=forms.CheckboxSelectMultiple(
            attrs={
                'class': 'bg-gray-50 dark:bg-gray-800 dark:text-gray-200 border border-gray-300 dark:border-gray-600 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-3'
            }
