...
class DetalleInstitucionFederal(models.Model):
    institucion_federal = models.ForeignKey(InstitucionesFederales, on_delete=models.CASCADE)  # Relaciona el detalle con una instsitucion federal; si la dependencia se elimina, también se elimina el detalle.
    comisaria_primera = models.ForeignKey(ComisariaPrimera, null=True, blank=True, on_delete=models.CASCADE, related_name='detalles_institucion_federal')  # Relaciona el detalle con la Comisaría Primera, permitiendo que quede vacío; si la comisaría se elimina, también se elimina el detalle.
    comisaria_segunda = models.ForeignKey(ComisariaSegunda, null=True, blank=True, on_delete=models.CASCADE, related_name='detalles_institucion_federal')  # Relaciona el detalle con la Comisaría Segunda, permitiendo que quede vacío; si la comisaría se elimina, también se elimina el detalle.
    comisaria_tercera = models.ForeignKey(ComisariaTercera, null=True, blank=True, on_delete=models.CASCADE, related_name='detalles_institucion_federal')  # Relaciona el detalle con la Comisaría Tercera, permitiendo que quede vacío; si la comisaría se elimina, también se elimina el detalle.
    comisaria_cuarta = models.ForeignKey(ComisariaCuarta, null=True, blank=True, on_delete=models.CASCADE, related_name='detalles_institucion_federal')  # Relaciona el detalle con la Comisaría Cuarta, permitiendo que quede vacío; si la comisaría se elimina, también se elimina el detalle.
    comisaria_quinta = models.ForeignKey(ComisariaQuinta, null=True, blank=True, on_delete=models.CASCADE, related_name='detalles_institucion_federal')  # Relaciona el detalle con la Comisaría Quinta, permitiendo que quede vacío; si la comisaría se elimina, también se elimina el detalle.
    numero_movil_federal = models.CharField(max_length=20, null=True, blank=True)  # Campo para almacenar el número de móvil de la dependencia provincial, permitiendo que quede vacío.
    nombre_a_cargo_federal = models.CharField(max_length=255, null=True, blank=True)  # Campo para almacenar el nombre de la persona a cargo de la dependencia provincial, permitiendo que quede vacío.
